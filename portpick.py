...
An app that refuses to open because some other program happens to be
on 8300 is an app that is not there when it is wanted. Worse: the
thing on 8300 is very often THIS APP, still running from before -- so the
...
MAX_TRIES = 16          # 8300 through 8315, then the OS decides

# the string that identifies THIS app, used only to tell "another copy of
# gvoices" from "some other program" in the startup message
_SELF_MARKER = b"GVoices, Google Voices"
