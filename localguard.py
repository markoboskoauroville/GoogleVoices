...
GVoices has real POST endpoints (/api/say spends a request of a key, /api/keys/delete rewrites the ring; the
...
uses. A GET-only guard would miss the actual new risk:
    ...
    http://127.0.0.1:8300 in the background. A POST that runs ffmpeg on
